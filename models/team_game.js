...
class TeamGame {
    ...
    static async report(id, lichessId) {
        const gameCheckRes = await db.query(`SELECT * FROM team_games WHERE id = $1`, [id]);
    ...
    static async forfeit(id, winner) {
        ...
        if (!gameCheck.result) {
            ...
            const winnerTeamRes = await db.query(`SELECT team FROM team_entries WHERE id = $1`, [winnerId]);
            const winnerTeam = winnerTeamRes.rows[0].team;
